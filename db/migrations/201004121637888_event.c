...
static int forward(Edi *db)
{
    ...
    if (!createEvent(db, "warning", "Port limit exceeded", "Port has exceeded configured transfer limit") ||
        !createEvent(db, "error", "Port stuck", "Port not operating anymore") ||
        !createEvent(db, "critical", "Port failed", "Port interface is not operational")) {
